...
> Output contract (JSON schema, inline formatting, page-count check): `docx_spec.md` — **read it first**.
> Shared rules (provenance, anti-fabrication): `CLAUDE.md`

---

## Output Mechanics (DOCX)

The cover letter is a JSON spec rendered to `.docx` by `build_docx.js`. Both files are deliverables; the
JSON is the editable source — `/edit-resume` edits the JSON and re-runs the build, never the `.docx`.

```json
{
  "format": "cover_letter",
  "sender": ["Name", "Street", "City, ST ZIP"],
  "date": "July 30, 2026",
  "recipient": ["Hiring Team", "Company", "City, ST"],
...
```bash
node resume_builder/helpers/build_docx.js output/<Folder>/<name>_cover_letter.json -o output/<Folder>/<name>_cover_letter.docx
soffice --headless --convert-to pdf --outdir output/<Folder> output/<Folder>/<name>_cover_letter.docx
pdfinfo output/<Folder>/<name>_cover_letter.pdf | grep Pages
```

Inline formatting inside `body` paragraphs: `**bold**`, `*italic*`, `[label](url)`, and literal Unicode for
dashes (`–`, `—`) and symbols (`°`, `β`, `%`). No LaTeX — no `\textbf{}`, `$...$`, `\href{}`.
...
- Verify the page count with the `soffice` + `pdfinfo` check above before declaring the CL done.
...
- Resume + CL = 3 pages | CV + CL = 5-6 pages
...
- Limit quantified claims to 3-5 per CL
- **Em-dashes in CLs:** Max 2 per document (write them as the literal `—`). CLs are prose-heavy and em-dashes compound quickly. Use commas for parenthetical asides, colons for elaborations, periods for new sentences. Paired em-dashes (X — detail — Y) should use commas or parentheses instead.
