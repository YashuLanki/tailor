...
Parse `$ARGUMENTS`: First argument is the `.json` spec path (required). A `.md` path is the critique file. Text in quotes is inline instructions.
- `/edit-resume output/Acme/e2e_acme_resume.json`
- `/edit-resume output/Acme/e2e_acme_resume.json output/Acme/critique_acme.md`
- `/edit-resume output/Acme/e2e_acme_resume.json "shorten Position 1 theme, fill last page"`

If given a `.docx` path, edit the sibling `.json` spec instead — the JSON is the editable source and the `.docx` is regenerated from it. Never edit a `.docx` directly.

If only the `.json` path and no instructions: ask the user what to fix.
...
Check `config.md` FIXED Sections for the list of locked sections (their JSON fragments live in `resume_builder/templates/fixed_sections.json`). Say no and explain: these are locked across all outputs.

Document spacing and layout live in `resume_builder/helpers/build_docx.js` — never edit them per document.
...
- If Edit N Status shows IN_PROGRESS: read the `.json` spec, identify which edits are done, resume
...
2. `resume_builder/reference/docx_spec.md` — JSON schema, inline markers, char budgets, page-count verification
3. `resume_builder/reference/resume_reference.md` — char limits, budgets, fixed sections
4. The `.json` spec being edited
5. Critique file (if provided in `$ARGUMENTS`)
6. JD file (path from session file's JD Info section)
7. Build the current spec and record baseline page count:
   ```bash
   node resume_builder/helpers/build_docx.js output/<FolderName>/[file].json -o output/<FolderName>/[file].docx
   soffice --headless --convert-to pdf --outdir output/<FolderName> output/<FolderName>/[file].docx
   pdfinfo output/<FolderName>/[file].pdf | grep Pages
   ```
8. Run: `python3 resume_builder/helpers/char_count.py -f output/<FolderName>/[file].json`

**Char budgets** come from `docx_spec.md` and supersede any numbers in `resume_reference.md`: Resume-1L 105–115 (max 122), Resume-2L 195–215 (max 230), CV-2L 175–190 (max 200), CV-3L 260–280 (max 295). Count the plain string with `**`/`*` markers removed.
...
If edit targets **cover letter** (not resume/CV): note this — Phase 4 will use CL-specific gates. Load the CL `.json` path from session file Output Files section.
...
Apply edits to the `.json` spec, one section at a time. After each edited section:
...
| Page count | Resume 2 pages, CV 4 (per `docx_spec.md` / `config.md` Output Rules) | Trim/expand VARIABLE content only |
| Orphan | 2L bullet last line >= 70% | Pad or trim |
| Theme width | Position theme + date fits 1 line (theme <= 62 chars) | Shorten theme |
| Build | `build_docx.js` exits clean | Fix the JSON spec (schema / missing keys) |
...
After all edits, rebuild and re-verify the page count:
```bash
node resume_builder/helpers/build_docx.js output/<FolderName>/[file].json -o output/<FolderName>/[file].docx
soffice --headless --convert-to pdf --outdir output/<FolderName> output/<FolderName>/[file].docx
pdfinfo output/<FolderName>/[file].pdf | grep Pages
```
Use the Read tool to view the PDF (`pdftoppm -jpeg -r 80 <pdf> page`) — check page count, white space, orphans, header wrapping.

Progress: "Editing Position 1 bullet 6 — was 184 chars, now 197..." / "Rebuilt — 2 pages, page fill OK"
...
5. **Present:** Changes summary + delta table + rebuilt `.docx` (and the PDF rendered for verification).
