...
Parse `$ARGUMENTS`:
- Session file path (e.g., `output/Acme/session_acme_engineer.md`) → read session file, derive `.json` spec paths from Output Files
- `.json` spec path(s) + JD source (existing format) → backward compatible
- Session name (e.g., `acme_engineer`) → find session file via derivation

If no CL `.json` provided or found in session file, critique resume/CV alone (Part 7 adjustments noted below).
...
FIXED sections (from `config.md` FIXED Sections, fragments in `resume_builder/templates/fixed_sections.json`) are locked — do not flag for editing. Flag only VARIABLE sections. Document spacing/layout lives in `resume_builder/helpers/build_docx.js` and is likewise not editable per document.
...
Find and read the session file for the spec being critiqued (use derivation protocol from shared_ops.md).
...
3. Read the `.json` spec file(s) — derive paths from session file Output Files, or from `$ARGUMENTS`
...
6. Run char count (budgets per `resume_builder/reference/docx_spec.md`):
   ```bash
   python3 resume_builder/helpers/char_count.py -f [file].json
   ```
7. Verify page count and visually inspect:
   ```bash
   ...
   ```
   Resume must be 2 pages, CV 4, cover letter 1. Use the Read tool to view the PDF (`pdftoppm -jpeg -r 80 <pdf> page`) — check orphans, page fill, header wrapping.
   If the build or PDF conversion fails: note "BUILD FAILED — visual checks could not be verified" in Part 8.
...
- session file, resume/CV `.json` + `.docx`, CL `.json` + `.docx`, critique `.md`
- Verification PDFs produced by the page-count gate
