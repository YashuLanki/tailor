...
4. **Recovery check:**
   ...
   - If CL Status is IN_PROGRESS → check if the CL `.json` spec exists, offer to resume or regenerate
...
2. **Finished resume/CV JSON spec** — path from session file Output Files. Read to understand what CL must complement.
...
Read `resume_builder/reference/docx_spec.md` — the `cover_letter` schema (`sender`, `date`, `recipient`, `salutation`, `body`, `closing`, `signature`), inline markers, and word budgets. Everything is plain Unicode; there is no markup to escape.
...
Save the JSON spec to `output/<FolderName>/e2e_<name>_cover_letter.json`
...
```bash
node resume_builder/helpers/build_docx.js output/<FolderName>/e2e_<name>_cover_letter.json -o output/<FolderName>/e2e_<name>_cover_letter.docx
```

Verify the page count per `docx_spec.md`:
```bash
soffice --headless --convert-to pdf --outdir output/<FolderName> output/<FolderName>/e2e_<name>_cover_letter.docx
pdfinfo output/<FolderName>/e2e_<name>_cover_letter.pdf | grep Pages
```

Use the Read tool to view the PDF (`pdftoppm -jpeg -r 80 <pdf> page`). Verify:

| Gate | Check | If FAIL |
|------|-------|---------|
| Word count | Industry 250-300, Lab/Academic 350-450 | Trim/expand |
| Page count | Cover letter: 1 page (CV package may run to 2 — see `config.md`) | Adjust content |
...
Update session file:
- Add CL to Output Files (both `.json` and `.docx`)
- Status: `Cover Letter: DONE`
- Add Next Critique command

Progress: "Built — 1 page, 278 words. Package cohesion verified."

### >>>>>> MANDATORY STOP — DO NOT PROCEED <<<<<<
Present: CL summary (word count, page count, key hooks used).
**You MUST wait for the user's explicit text response before continuing.**

If user requests changes: apply them to the `.json` spec, rebuild, re-verify. Update session file.
